...
@interface LJZRecordManager ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate> {
...
@property (strong, nonatomic) LJZRecordEncoder           *recordEncoder;//录制编码
...
@end
@implementation LJZRecordManager
...
- (void)takePhoto:(void(^)(UIImage *image))callback{
    ...
    if (connection.isVideoOrientationSupported) {
        connection.videoOrientation = [[LJZMotionManager sharedManager] currentVideoOrientation];
    }
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [LJZRecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
