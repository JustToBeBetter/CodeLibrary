...
#import "LJZRecordViewController.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "LJZRecordManager.h"
#import "LJZRecordSuccessPreview.h"
#import "LJZRecordProgressView.h"
...
#define VIDEO_MIN_TIME 2 // 录制视频最短时间

@interface LJZRecordViewController ()<LJZRecordEngineDelegate,UIGestureRecognizerDelegate,LJZMotionManagerDeviceOrientationDelegate>

@property (nonatomic ,strong) LJZRecordManager *recordManger;
...
@property (nonatomic ,strong) LJZRecordSuccessPreview *preview;// 拍摄成功预览视图
@property (nonatomic ,strong) LJZRecordProgressView *recordButton;// 录制按钮
...
@property (nonatomic ,strong) UILabel *alartLabel;// (提示)拍摄时间太短,不少于2s
@property (nonatomic ,strong) UIImage *coverImage;// 视频封面

@end

@implementation LJZRecordViewController
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    // 监听设备方向
    [[LJZMotionManager sharedManager] startDeviceMotionUpdates];
    [LJZMotionManager sharedManager].delegate = self;;
...
- (void)viewDidDisappear:(BOOL)animated{
    ...
    // 停止监听设备方向
    [LJZMotionManager sharedManager].delegate = nil;
    [[LJZMotionManager sharedManager] stopDeviceMotionUpdates];
#if __IPHONE_OS_VERSION_MAX_ALLOWED < __IPHONE_9_0
...
- (void)takephoto{
    ...
    [self.recordManger takePhoto:^(UIImage *image) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [strongSelf.preview setImage:image videoPath:nil captureVideoOrientation:[[LJZMotionManager sharedManager] currentVideoOrientation]];
        });
    }];
}
#pragma mark -
...
- (void)stopRecord:(BOOL)isSuccess{
    ...
    [self.recordManger stopCaptureWithStatus:isSuccess handler:^(UIImage *movieImage,NSString *filePath) {
        ...
        STRONGSELF
        strongSelf.coverImage = movieImage;
        dispatch_async(dispatch_get_main_queue(), ^{
            [strongSelf.recordManger shutdown];
            [strongSelf.preview setImage:nil videoPath:filePath captureVideoOrientation:[[LJZMotionManager sharedManager] currentVideoOrientation]];
        });
    }];
}
#pragma mark -
...
- (void)sendWithImage:(UIImage *)image videoPath:(NSString *)videoPath{
    NSLog(@"发送");
    if (self.recordBlock) {
        self.recordBlock(image, videoPath,self.coverImage);
    }
...
- (LJZRecordManager *)recordManger {
    if (!_recordManger) {
        _recordManger = [[LJZRecordManager alloc] init];
...
- (LJZRecordSuccessPreview *)preview{
    if (!_preview) {
        _preview = [[LJZRecordSuccessPreview alloc]initWithFrame:self.view.bounds];
        WEAKSELF
...
- (void)setupUI{
    ...
    [exitButton setImage:[UIImage imageNamed:@"短视频_关闭"] forState:UIControlStateNormal];
    [exitButton addTarget:self action:@selector(exitRecordController) forControlEvents:UIControlEventTouchUpInside];
    exitButton.frame = CGRectMake(0, 0, 32,32);
    exitButton.center = CGPointMake(self.view.center.x-156/4 - 32/2 - 40, self.view.bounds.size.height - 97);
    [self.view addSubview:exitButton];
    _exitButton = exitButton;
    
    // 录制按钮
    LJZRecordProgressView *recordButton = [[LJZRecordProgressView alloc]initWithFrame:CGRectMake(0, 0, 156/2, 156/2)];
    ...
    [switchButton setImage:[UIImage imageNamed:@"短视频_翻转"] forState:UIControlStateNormal];
    [switchButton addTarget:self action:@selector(switchCamara:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark -
#pragma mark -LJZRecordEngineDelegate(录制进度回调)
...
#pragma mark -
#pragma mark -LJZMotionManagerDeviceOrientationDelegate --> 控制按钮方向
