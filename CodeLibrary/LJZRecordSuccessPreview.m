...
@interface LJZRecordSuccessPreview(){
    float _width;
    float _distance;
}
...
@implementation LJZRecordSuccessPreview
...
- (void)setupUI{
    ...
    [cancelButton setImage:[UIImage imageNamed:@"短视频_重拍"] forState:UIControlStateNormal];
    [cancelButton addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchDragInside];
    ...
    [sendButton setImage:[UIImage imageNamed:@"短视频_完成"] forState:UIControlStateNormal];
    [sendButton addTarget:self action:@selector(send) forControlEvents:UIControlEventTouchDragInside];
