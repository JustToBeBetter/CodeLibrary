...
@implementation YKGPUImagePicInput
...
- (BOOL)processPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    return [self processPixelBuffer:pixelBuffer time:kCMTimeIndefinite];
}

- (BOOL)processPixelBuffer:(CVPixelBufferRef)pixelBuffer time:(CMTime)frameTime {
    return [self processPixelBuffer:pixelBuffer time:frameTime completion:nil];
}
