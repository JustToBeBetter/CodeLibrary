...
@implementation YKFaceStickersManager {
...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&onceToken, ^{
        _stickersManager = [YKFaceStickersManager new];
    });
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _ioQueue = dispatch_queue_create("com.feng.stickers", DISPATCH_QUEUE_SERIAL);
        _stickerPath =
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:_stickerPath]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:_stickerPath withIntermediateDirectories:NO
...
- (void)updateConfigJSON {
    [self updateConfigJSONForDict];
}

//Update the download information for all stickers
- (NSMutableDictionary *)updateConfigJSONForDict {
    NSDictionary *oldDict = [NSDictionary dictionaryWithContentsOfFile:[_stickerPath
            stringByAppendingPathComponent:@"stickers.json"]];
    ...
    for (NSDictionary *itemDict in oldArr) {
        ...
        dictionary[@"name"] = [itemDict valueForKey:@"name"];
        dictionary[@"dir"] = [itemDict valueForKey:@"dir"];
        dictionary[@"category"] = [itemDict valueForKey:@"category"];
        dictionary[@"thumb"] = [itemDict valueForKey:@"thumb"];
        dictionary[@"voiced"] = [itemDict valueForKey:@"voiced"];
        dictionary[@"sourceType"] = [itemDict valueForKey:@"sourceType"];

        if ([[NSFileManager defaultManager] fileExistsAtPath:[_stickerPath stringByAppendingPathComponent:[itemDict valueForKey:@"dir"]]]) {
            dictionary[@"downloaded"] = [NSNumber numberWithBool:YES];
        } else {
            dictionary[@"downloaded"] = [NSNumber numberWithBool:NO];
        }
        [newArr addObject:dictionary];
    }
...
- (BOOL)getStickersFromJson {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:[_stickerPath stringByAppendingPathComponent:@"stickers.json"]
                                              isDirectory:NULL]) {
        [oldDict writeToFile:[_stickerPath stringByAppendingPathComponent:@"stickers.json"] atomically:NO];
    }

    //拷贝本地贴纸到沙盒
    NSString *localPath = [[[NSBundle mainBundle] pathForResource:@"Stickers" ofType:@"bundle"] stringByAppendingPathComponent:@"stickers"];
    ...
    for (NSString *stickerName in dirArr) {
        if (self.isLoadStickersFromServer) {
            if (![[NSFileManager defaultManager] fileExistsAtPath:[_stickerPath stringByAppendingPathComponent:stickerName]]) {
                [[NSFileManager defaultManager] copyItemAtPath:[localPath stringByAppendingPathComponent:stickerName]
                                                        toPath:[_stickerPath stringByAppendingPathComponent:stickerName]
                                                         error:NULL];
            }
        } else {
            [[NSFileManager defaultManager] copyItemAtPath:[localPath stringByAppendingPathComponent:stickerName]
    ...
    for (NSDictionary *itemDict in newArr) {
        ...
        YKFaceSticker *sticker = [[YKFaceSticker alloc] initWithName:[itemDict valueForKey:@"name"]
                thumbName:[itemDict valueForKey:@"thumb"] download:[[itemDict valueForKey:@"downloaded"] boolValue] directoryURL:url];
        if ([[itemDict valueForKey:@"sourceType"] intValue] == 0) {
            sticker.sourceType = YKFaceStickerSourceTypeFromRemote;
        } else {
            sticker.sourceType = YKFaceStickerSourceTypeFromLocal;
        }
 ...
 */
- (void)updateStickersJSONWithCompletion:(void (^)(BOOL isSuccess, NSMutableDictionary *dic))completion
                              serverJson:(NSArray *)serverJson {
    BOOL isSuccess = NO;
    ...
    newDict = [self updateConfigJSONForDict];
